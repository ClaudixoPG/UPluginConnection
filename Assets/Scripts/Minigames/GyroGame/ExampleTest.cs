...
public class ExampleScript : MonoBehaviour
{
    ...
    // An optional offset quaternion to adjust for the initial phone orientation
    ...
    IEnumerator InitializeGyro()
    {
        ...
        if (SystemInfo.supportsGyroscope)
        {
            ...
            // Wait one frame for the gyro to start providing data
            yield return null;

            // Capture the initial orientation to use as an offset if needed
            // The default attitude might not be (0,0,0,0) at start, so this sets a baseline.
            rotationOffset = Quaternion.Inverse(gyro.attitude);

            Debug.LogWarning("Gyro Enabled and Initialized");
        }
        else
        {
    ...
    {
        GyroModifyCamera();
        //print all the gyro values for debugging in one single line, so we can see them all together as they change
        Debug.Log("Gyro Attitude: " + Input.gyro.attitude + " | Gyro Rotation Rate: " + Input.gyro.rotationRate + " | Gyro Gravity: " + Input.gyro.gravity + " | Gyro User Acceleration: " + Input.gyro.userAcceleration + " | Gyro Enabled: " + gyroEnabled + " | Screen Orientation: " + Screen.orientation + " | Screen Width: " + Screen.width + " | Screen Height: " + Screen.height);
    }
